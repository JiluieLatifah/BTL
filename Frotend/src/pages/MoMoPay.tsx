...
export const MoMoPay: React.FC = () => {
  ...
  // Nếu chưa chọn phòng khám hoặc dịch vụ, tự động quay lại danh sách
  useEffect(() => {
    if (!bookingState.selectedClinic || !bookingState.service) {
      navigate('/clinics');
    }
  }, [bookingState.selectedClinic, bookingState.service, navigate]);
  ...
  // Quản lý trạng thái: 'scanning' (đang chờ quét) | 'success' (quét thành công)
  const [status, setStatus] = useState<'scanning' | 'success'>('scanning');
  const [timer, setTimer] = useState(600); // 10 phút đếm ngược (đơn vị: giây)

  // Bộ đếm ngược thời gian cho giai đoạn quét mã QR
  useEffect(() => {
  ...
  // Giả lập luồng: Tự động chuyển từ màn hình quét sang trạng thái thành công sau 6 giây
  useEffect(() => {
    if (status !== 'scanning') return;
  ...
  // 🌟 ĐỒNG BỘ LUỒNG TỰ ĐỘNG TRIGGER KHI THANH TOÁN THÀNH CÔNG
  useEffect(() => {
    if (status !== 'success') return;

    const successTimeout = setTimeout(async () => {
      await handleFinalize(); // Chờ xử lý đẩy API xuống PostgreSQL xong mới chuyển trang
    }, 4000);

    return () => clearTimeout(successTimeout);
  }, [status]);

  // 🌟 XỬ LÝ ĐỒNG BỘ LƯU XUỐNG DATABASE POSTGRESQL QUA BIẾN ASYNC/AWAIT
  const handleFinalize = async () => {
    try {
      await generateTicket(); // Đợi Backend xử lý Insert DB thành công và trả số phiếu thật về Context
      navigate('/ticket-detail');
    } catch (error: any) {
      console.error('Lỗi khi xử lý lưu phiếu khám hệ thống:', error);
      // Hiển thị trực tiếp thông báo lỗi từ Backend (ví dụ: bị trùng lịch khám do Optimistic Locking)
      alert(error.message || 'Hệ thống bận hoặc khung giờ đặt khám đã bị trùng do xung đột dữ liệu. Vui lòng tải lại trang!');
      // 🌟 CHẶN ĐỨNG điều hướng bừa bãi, giữ người dùng lại màn hình MoMo để xử lý hoặc quay lại sau
    }
  };
    ...
    <div className="min-h-screen bg-[#F3F4F6] flex flex-col font-sans">
      ...
      <div className="bg-amber-500 text-white text-xs font-bold text-center py-2 px-4 shadow-inner flex items-center justify-center gap-2">
        ...
        {status === 'scanning' ? (
          ...
          <span>[GIẢ LẬP]: Thanh toán thành công! Sẽ tự động gọi API lưu DB và chuyển hướng sau 4 giây...</span>
        )}
      </div>
          ...
          <div className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden w-full p-8 md:p-12 text-center flex flex-col items-center gap-6">
            ...
            <div className="space-y-4 pt-4 border-t border-gray-100 w-full">
              <p className="text-xs text-gray-400 font-medium">
                Hệ thống đang đồng bộ bản ghi và tự động chuyển hướng về trang bệnh viện.
              </p>
