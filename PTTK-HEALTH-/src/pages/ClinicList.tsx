...
export const ClinicList: React.FC = () => {
  const { bookingState, selectClinic } = useBooking();
        ...
        {filteredClinics.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredClinics.map((clinic) => (
              ...
              >
                <div>
                  {/* Clinic logo / image placeholder - User can add clinic images in assets */}
                  ...
                  {/* Body Info */}
                  <div className="p-6">
                    <h3 className="font-bold text-gray-900 text-lg mb-1 leading-snug group-hover:text-radiant">
                      {clinic.name}
                    </h3>
                ...
                <div className="px-6 pb-6 pt-0 grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="py-2.5 px-4 text-xs font-bold border border-radiant text-radiant rounded-xl hover:bg-radiant-light active:scale-[0.98] transition-all text-center"
                  >
                    Xem chi tiết
                  </button>
                  <button
                    onClick={() => handleBooking(clinic)}
                    className="py-2.5 px-4 text-xs font-bold bg-radiant text-white rounded-xl hover:bg-radiant-dark shadow-sm hover:shadow-md active:scale-[0.98] transition-all text-center"
                  >
                    Đặt khám ngay
                  </button>
                </div>
